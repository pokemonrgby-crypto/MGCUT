...
// functions/lib/prompts.mjs loadCharacterBasePrompt 함수 내부
...
// (기존 JSON 스키마)
{
  ...
  // [수정] chosen 필드를 이름(문자열)에서 인덱스(숫자)로 변경
  "chosen": ["선택한 어빌리티 3개의 인덱스(숫자)"],
  "items": [
...
// [수정] chosen 규칙을 인덱스로 명시
- chosen 배열은 abilities 중 3개를 골라 그 인덱스(0~5)로 채워야 합니다.
