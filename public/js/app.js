...
$('#btn-logout').onclick = async () => { await signOut(auth); await render(); };
}
...
if(!id) return;
try{ await api.likeWorld(id); await renderWorlds(); }catch(err){ alert(err.message); }
};
}catch(err){ root.innerHTML = '목록 로딩 실패: ' + err.message; }
}
